...
class Autopilot {
 public:
  ...
  bool isFlying() const;
  bool isLanded() const;
  bool isPathClear(const cv::Mat& map,const arp::kinematics::RobotState& X,double xd,double yd,double zd);
  void setoccupancymap(const cv::Mat& map);
 ...
 protected:
  ...
  cv::Mat map_;
  PidController pidX_; ///< PID controller for x position.
  PidController pidY_; ///< PID controller for y position.
  PidController pidZ_; ///< PID controller for z position.
  PidController pidYaw_; ///< PID controller for yaw angle.
  arp::kinematics::RobotState x_; ///< The current state of the drone.
};
